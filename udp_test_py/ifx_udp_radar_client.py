        ...
        while True:
                try:
                        ...
                        if all(received_packets[frame_num]): 
                                ...
                                #with open(path+"/"+str(counter)+".pkl", 'wb') as f:
                                        # Use the pickle.dump function to serialize the data
                                        #pickle.dump(radar_data, f)
                                
                                #np.savetxt(path+"/"+str(counter)+".txt", radar_data.flatten(), delimiter=',', fmt='%f')

                                # Save plot images for each antenna 
                                plt.imsave(f'{path}/ant1_psoc_doppler_{counter}.jpg', radar_data[0, :, :], format='png') 
                                plt.imsave(f'{path}/ant2_psoc_doppler_{counter}.jpg', radar_data[1, :, :], format='png') 
                                plt.imsave(f'{path}/ant3_psoc_doppler_{counter}.jpg', radar_data[2, :, :], format='png')
                                counter += 1 
