...
def udp_client_radar(server_ip, server_port):
        ...
        # Apri i file CSV per scrivere i dati 
        csv_file1 = open(path + "/antenna1_data.csv", 'w', newline='') 
        csv_writer1 = csv.writer(csv_file1) 
        csv_writer1.writerow(['Time (seconds)'] + [f'antenna1_{i}' for i in range(32*32)]) 
        csv_file2 = open(path + "/antenna2_data.csv", 'w', newline='') 
        csv_writer2 = csv.writer(csv_file2) 
        csv_writer2.writerow(['Time (seconds)'] + [f'antenna2_{i}' for i in range(32*32)]) 
        csv_file3 = open(path + "/antenna3_data.csv", 'w', newline='') 
        csv_writer3 = csv.writer(csv_file3) 
        csv_writer3.writerow(['Time (seconds)'] + [f'antenna3_{i}' for i in range(32*32)]) 
        ...
        while True:
                try:
                        ...
                        if all(received_packets[frame_num]):
                                ...
                                # Calcola il tempo in secondi dall'inizio
                                elapsed_time = time.time() - start_time

                                #Salva i dati delle antenne nei file CSV 
                                antenna1_data = radar_data[0].flatten() 
                                antenna2_data = radar_data[1].flatten() 
                                antenna3_data = radar_data[2].flatten() 
                                csv_writer1.writerow([elapsed_time] + antenna1_data.tolist()) 
                                csv_writer2.writerow([elapsed_time] + antenna2_data.tolist()) 
                                csv_writer3.writerow([elapsed_time] + antenna3_data.tolist())

                                counter += 1
                                del received_packets[frame_num]  # Remove completed frame from dictionary
