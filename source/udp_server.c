...
void udp_server_task(void *arg)
{
    ...
    printf(" 'subscriber_task_q'\n\n");
    if (radar_data_queue == NULL)
    {
        printf(" 'subscriber_task_q' queue creation failed... Task suspended\n\n");
        CY_ASSERT(0);
    }

    radar_config_queue = xQueueCreate(TASK_QUEUE_LENGTH, sizeof(sub_msg));
    printf(" 'radar_config_queue'\n\n");
    ...
    printf(" 'radar_task before'\n\n");

    //if(pdPASS !=  xTaskCreate(radar_task, "radar_task", configMINIMAL_STACK_SIZE * 8, NULL, 7, &radar_task_handle))
	if(pdPASS !=  xTaskCreate(radar_task, "radar_task", RADAR_TASK_STACK_SIZE , NULL, RADAR_TASK_PRIORITY, &radar_task_handle))
	{
        ...
        if (pdTRUE ==  xQueueReceive( radar_data_queue, &msg, ( TickType_t ) portMAX_DELAY ))
        {
            switch(msg->cmd)
            {
                case RADAR_DATA_COMMAND:
                {
                    ...
                    /*for (int8_t sample_idx = 0; sample_idx < 128; ++sample_idx)
										  {
						//publisher_msg->data[6+sample_idx] = (uint8_t) samples[sample_idx];
						printf("data[%d] = %d...\n\n",sample_idx, msg->data[6+sample_idx]);
					}*/
                    break;
                }
            }
    ...
    /*while(true){
		result = cy_socket_sendto(server_radar_data, (uint8) 1, 1, CY_SOCKET_FLAGS_NONE,
													 &peer_addr, sizeof(cy_socket_sockaddr_t), &bytes_sent);
		if(result == CY_RSLT_SUCCESS )
		{
			printf("Data with length:%" PRIu32 " sent to udp client\n", bytes_sent);
